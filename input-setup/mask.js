...
function run() {
  try {
    ...
    const envVarConfigs = [
      ...
      {
        varname: "INPUT_ARGUMENTS",
        payloadValue: payload?.arguments ?? "",
        backup: inputs.arguments ?? "",
      },
      ...
      {
        varname: "INPUT_CACHE_PATH",
        payloadValue: payload?.cachePath ?? "",
        backup: "~/.cache/trunk",
      },
      {
    ...
    envVarConfigs.forEach(({ varname, payloadValue, backup }) => {
      const toWrite = usePayload ? payloadValue : backup;
      if (toWrite !== undefined) {
        fs.writeSync(githubEnv, Buffer.from(`${varname}=${toWrite}\n`));
      }
    });
